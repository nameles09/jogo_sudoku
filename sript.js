...
function paintCells(element, firstIndex, secondIndex, elementCase) {
  switch (elementCase) {
    case "row":
      document
        .querySelectorAll("tr")
        [element].children[firstIndex].classList.add("error");
      document
        .querySelectorAll("tr")
        [element].children[secondIndex].classList.add("error");

      break;

    case "column":
      document
        .querySelectorAll(`table tr > td:nth-child(${element + 1})`)
        [firstIndex].classList.add("error");
      document
        .querySelectorAll(`table tr > td:nth-child(${element + 1})`)
        [secondIndex].classList.add("error");

      break;

    case "block":
      document
        .querySelectorAll(`.block${element}`)
        [firstIndex].classList.add("error");
      document
        .querySelectorAll(`.block${element}`)
        [secondIndex].classList.add("error");
...
function generateBoard(boardNumbers) {
  boardNumbers.forEach((row, rowKey) => {
    document
      .querySelectorAll("tr")
      [rowKey].querySelectorAll("td")
      .forEach(function (cell, cellKey) {
